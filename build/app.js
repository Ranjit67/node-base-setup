    ...
    middleware(middleware, st) {
        middleware.forEach((middle) => {
            if (middle.includes(st)) {
                Promise.resolve().then(() => __importStar(require(path_1.default.join(__dirname + "/middleware/" + middle)))).then((middleReader) => {
    ...
    routes() {
        ...
        subRoutes.forEach((file) => {
            if (file.includes(".routes.")) {
                ...
                Promise.resolve().then(() => __importStar(require(path_1.default.join(__dirname + "/routes/" + file)))).then((route) => {
